...
echo "[render_medium] output=$OUTPUT_DIR workers=$N_WORKERS"
python dataset_gen/scripts/make_paired_dataset_fast.py \
    --maze medium \
    --output_dir "$OUTPUT_DIR" \
    --n_workers "$N_WORKERS"

echo "[validate_medium]"
PYTHONPATH=src python dataset_gen/scripts/validate_antmaze_dataset.py \
    --zarr_dir "$OUTPUT_DIR/medium" \
    --output_dir "$OUTPUT_DIR/medium/validation" \
    --maze medium \
    --n_verify_eps 3

echo "[done] medium → $OUTPUT_DIR/medium"
